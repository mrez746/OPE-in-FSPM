best_regret_2 <- function(pi_l, pi_t, k = 10, ...) {
  # function for finding the best f matrix wrt regret
  # input:
  ...
  constr1 <- mu * e_vec >= A %*% square(f) - t(S2_mat) %*% pi_l
  constr2 <- A %*% f == t(T_mat) %*% pi_t
  constr3 <- f[k] == 0
