best_minmax_regret <- function(pi_l, pi_t, rho, k = 10, ...) {
  ...
  mat_1  <- expm::sqrtm(Pi_inv - 1) %*% t(B) %*% diag(A[1, ])
  mat_2  <- expm::sqrtm(Pi_inv - 1) %*% t(B) %*% diag(A[2, ])
  mat_3  <- expm::sqrtm(Pi_inv - 1) %*% t(B) %*% diag(A[3, ])
  mat_4  <- expm::sqrtm(Pi_inv - 1) %*% t(B) %*% diag(A[4, ])
  mat_5  <- expm::sqrtm(Pi_inv - 1) %*% t(B) %*% diag(A[5, ])
  mat_6  <- expm::sqrtm(Pi_inv - 1) %*% t(B) %*% diag(A[6, ])
  mat_7  <- expm::sqrtm(Pi_inv - 1) %*% t(B) %*% diag(A[7, ])
  mat_8  <- expm::sqrtm(Pi_inv - 1) %*% t(B) %*% diag(A[8, ])
  mat_9  <- expm::sqrtm(Pi_inv - 1) %*% t(B) %*% diag(A[9, ])
  mat_10 <- expm::sqrtm(Pi_inv - 1) %*% t(B) %*% diag(A[10, ])
